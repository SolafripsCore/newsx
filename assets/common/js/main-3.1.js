...
(function ($) {
    ...
    function initCategoryTabs() {
        $(document).on('click', '.ajax-category-tab', function (e) {
            ...
            $.ajax({
                url: generateUrl('Ajax/loadPostsByCategory'),
                type: 'POST',
                dataType: 'json',
                data: {
                    category_id: catId,
...
(function ($) {
    ...
    $(document).on('click', '.btn-cancel-bank-transfer', function () {
        let transactionId = $(this).attr('data-id');
        Swal.fire({
            text: VrConfig.text.cancelOrder,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: VrConfig.text.yes,
            cancelButtonText: VrConfig.text.cancel,
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: 'POST',
                    url: generateUrl('Payment/cancelOrder'),
                    data: data,
                    dataType: 'json',
                    success: function (res) {
                        if (res) {
