(function ($) {
    ...
    const aiDOM = {
        ...
        inputType: $('input[name="ai_generate_type"]'),
    ...
    const AiWriterApp = {
        ...
        handleGeneration: function ($btn, isRegen) {
            ...
            const requestData = {
                ...
                'generate_type': aiDOM.inputType.val(),
        ...
        transferContentToMainForm: function () {
            ...
            // Select ALL matching elements globally on the page for inputs
            const $targetTitles = $(AiWriterConfig.selectors.targetTitle);
            const $targetSummaries = $(AiWriterConfig.selectors.targetSummary);
            const $targetKeywords = $(AiWriterConfig.selectors.targetKeywords);

            // Transfer Title to ALL matched elements
            if (generatedTitle && $targetTitles.length) {
                $targetTitles.each(function () {
                    $(this).val(generatedTitle).trigger('change').trigger('input');
                });
            }

            // Transfer Summary to ALL matched elements
            if (generatedSummary && $targetSummaries.length) {
                $targetSummaries.each(function () {
                    $(this).val(generatedSummary).trigger('change').trigger('input');
                });
            }

            // Transfer Keywords to ALL matched elements (with Tagify support)
            if (generatedKeys && $targetKeywords.length) {
                $targetKeywords.each(function () {
                    const $el = $(this);
                    if ($el.data('tagify')) {
                        try {
                            $el.data('tagify').addTags(generatedKeys);
                        } catch (e) {
                            console.warn('Tagify error:', e);
                        }
                    } else {
                        $el.val(generatedKeys).trigger('change');
                    }
                });
            }

            // Transfer Rich Text Content (Remains scoped to a single editor)
