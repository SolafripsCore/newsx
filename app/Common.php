...
if (!function_exists('postHasImage')) {
    /**
     * Determines whether a given post object has an associated image.
     *
     * @param object $post The post object containing data fields (e.g., image_id, image_url, feed_id).
     * @return bool True if an image identifier or URL is present; otherwise false.
     */
    function postHasImage(?object $post): bool
    {
        ...
        return !empty($post->image_id) || !empty($post->image_url) || !empty($post->feed_id);
    }
}
...
if (!function_exists('formatDateClient')) {
    ...
    function formatDateClient(string|int|null $timestamp): ?string
    {
        ...
        $settings = getContextValue('settings') ?? null;

        $format = !empty($settings) && !empty($settings->date_format) ? $settings->date_format : 'M d, Y';
...
if (!function_exists('formatDate')) {
    /**
     * Formats a date with time in a fixed numeric format for backend/admin tables
     *
...
if (!function_exists('getFeedFallbackErrorAttr')) {
    /**
     * Returns the complete 'onerror' HTML attribute for the feed fallback image.
     * Uses a static variable to prevent redundant context fetching within loops.
     *
     * @param object|null $post Post object containing the feed_id
     * @param string $imageSize Image size key (e.g., "small", "mid", "big")
     * @return string The ready-to-use onerror attribute, or an empty string
     */
    function getFeedFallbackErrorAttr(?object $post, string $imageSize): string
    {
        if (empty($post) || empty($post->feed_id)) {
            return '';
        }

        static $feedFallbackImages = null;

        if ($feedFallbackImages === null) {
            $feedFallbackImages = getContextValue('feedFallbackImages');
        }

        if (empty($feedFallbackImages) || empty($feedFallbackImages[$post->feed_id])) {
            return '';
        }

        $image = $feedFallbackImages[$post->feed_id];
        $sizeKey = "image_{$imageSize}";

        // Construct the URL and return the fully formatted HTML attribute
        if (!empty($image->{$sizeKey})) {
            $storage = $image->storage ?? 'local';
            $fallbackUrl = getStorageFileUrl($image->{$sizeKey}, $storage);

            if (!empty($fallbackUrl)) {
                // Return the complete safely escaped onerror attribute
                return 'onerror="this.onerror=null; this.src=\'' . esc($fallbackUrl) . '\';"';
            }
        }
...
if (!function_exists('getPremiumBadge')) {
    /**
     * Returns the universal access badge (Crown) for paid or exclusive content
     *
     * @param object|null $post The post object containing premium status properties
     * @param string $class Optional additional CSS classes to append to the wrapper
     * @param bool $isShifted True if the premium badge needs to be pushed left (e.g., next to an event badge)
     * @return string Returns the HTML string for the badge
     */
    function getPremiumBadge($post, string $class = '', bool $isShifted = false): string
    {
        // Cache the settings statically to fetch them only once per request for performance
        static $settingsCache = null;

        if ($settingsCache === null) {
            $settings = getContextValue('premiumMembership') ?? null;

            $settingsCache = [
                'subSystem'       => (int)($settings->subscriptionStatus ?? 0),
                'subMode'         => $settings->subscriptionMode ?? '',
                'excStatus'       => (int)($settings->exclusiveSaleStatus ?? 0),
                'badgeVisibility' => (int)($settings->premiumBadgeVisibility ?? 0)
            ];
        }

        if (!$settingsCache['badgeVisibility']) {
            return '';
        }

        // If neither the subscription system nor the exclusive sale status is active, return early
        if (!$settingsCache['subSystem'] && !$settingsCache['excStatus']) {
            return '';
        }

        $postObj = (object)$post;
        $isExclusive = !empty($postObj->is_exclusive) ? (int)$postObj->is_exclusive : 0;
        $isPremium = !empty($postObj->is_premium) ? (int)$postObj->is_premium : 0;

        $badgeType = '';

        // Exclusive badge takes precedence over the premium badge
        if ($isExclusive && $settingsCache['excStatus']) {
            $badgeType = 'exclusive';
        } // If it's not exclusive but premium system is active
        elseif ($isPremium && $settingsCache['subSystem']) {
            if ($settingsCache['subMode'] !== 'all') {
                $badgeType = 'premium';
            }
        }

        // If no conditions are met (or badge is hidden due to 'all' mode), return empty string
        if (empty($badgeType)) {
            return '';
        }

        // Change only the tooltip title based on the specific access type
        $title = ($badgeType === 'exclusive') ? trans("exclusive_content") : trans("premium_content");

        // Add the shifted class if there is an event badge next to it
        $shiftClass = $isShifted ? ' access-badge-shifted' : '';
        $finalClass = trim('access-badge ' . esc($class) . $shiftClass);

        // Return the badge with the inline universal solid Crown icon
        return '<div class="' . $finalClass . '" title="' . esc($title) . '">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor" class="badge-icon">
                        <path d="M2.80577 5.20006L7.00505 7.99958L11.1913 2.13881C11.5123 1.6894 12.1369 1.58531 12.5863 1.90631C12.6761 1.97045 12.7546 2.04901 12.8188 2.13881L17.0051 7.99958L21.2043 5.20006C21.6639 4.89371 22.2847 5.01788 22.5911 5.47741C22.7228 5.67503 22.7799 5.91308 22.7522 6.14895L21.109 20.1164C21.0497 20.62 20.6229 20.9996 20.1158 20.9996H3.8943C3.38722 20.9996 2.9604 20.62 2.90115 20.1164L1.25792 6.14895C1.19339 5.60045 1.58573 5.10349 2.13423 5.03896C2.37011 5.01121 2.60816 5.06832 2.80577 5.20006Z"></path>
                    </svg>
                </div>';
    }
}
...
if (!function_exists('getMediaIcon')) {
    /**
     * Returns the appropriate media icon (SVG) or event date badge for a given post type.
     *
...
if (!function_exists('getPostBadges')) {
    /**
     * Orchestrates and returns all relevant badges and icons for a post.
     * This is the single function you should call in your view files.
     *
     * @param object|null $post The post object
     * @param string $class Optional additional CSS classes for the media/event icon
     * @return string Returns the concatenated HTML string of all badges
     */
    function getPostBadges($post, string $class = ''): string
    {
        $html = '';
        $postObj = (object)$post;

        // 1. Generate Media/Event Icon first to check its existence
        $mediaIconHtml = getMediaIcon($postObj, $class);

        // 2. Check if the post is an event AND successfully rendered an event badge
        $isEvent = isset($postObj->post_format) && $postObj->post_format === 'event';
        $hasEventBadge = $isEvent && !empty($mediaIconHtml);

        // 3. Generate Premium/Exclusive Badge, telling it to shift left ONLY if an event badge is present
        $premiumBadgeHtml = getPremiumBadge($postObj, $class, $hasEventBadge);

        // Append to output in the requested DOM order: Premium first, then Event
        if (!empty($premiumBadgeHtml)) {
            $html .= $premiumBadgeHtml;
        }

        if (!empty($mediaIconHtml)) {
            $html .= $mediaIconHtml;
        }
...
if (!function_exists('getFontLineHeight')) {
    /**
     * Calculates a fluid and optically balanced line-height based on font size.
     * Caps small sizes (<= 16px) at a standard 1.5 for UI consistency (Bootstrap style).
     *
     * @param int|float|string $px Font size in pixels
     * @param bool $isContent Whether the text is long-form content
     * @return float Unitless line-height ratio, rounded to 3 decimal places
     */
    function getFontLineHeight(int|float|string $px, bool $isContent = false): float
    {
        $size = (float)$px;

        // Prevent division by zero or exceptionally small font sizes
        if ($size <= 0) {
            return $isContent ? 1.6 : 1.5;
        }

        // The curve constant controls the transition
        $curveConstant = $isContent ? 10.4 : 8.0;

        // Typographic boundaries
        $minLineHeight = 1.1; // Large display headings

        // Standard UI will never exceed 1.5. Long content can stretch to 1.65.
        $maxLineHeight = $isContent ? 1.65 : 1.50;

        // Core fluid typography formula
        $calculatedRatio = 1.0 + ($curveConstant / $size);

        // Clamp the result so it stops exactly at 1.5 for small fonts
        $ratio = max($minLineHeight, min($maxLineHeight, $calculatedRatio));
...
if (!function_exists('getPaywallType')) {
    /**
     * Returns selected paywall type
     */
    function getPaywallType(): string
    {
        $type = 'preview';

        $settings = getContextValue('premiumMembership') ?? null;

        if (!empty($settings) && !empty($settings->paywallType) && $settings->paywallType == 'hard') {
            $type = 'hard';
        }
