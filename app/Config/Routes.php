...
$routes->group('webhook', function ($routes) {
    ...
    $routes->post('paystack', 'WebhookController::paystack');
    ...
    $routes->post('nowpayments', 'WebhookController::nowpayments');
});
...
$routes->group('checkout/payment', function ($routes) {
    ...
    $routes->get('paystack', 'PaymentController::paystack');
    ...
    $routes->match(['GET', 'POST'], 'nowpayments', 'PaymentController::nowpayments');
    $routes->post('bank-transfer', 'PaymentController::bankTransfer');
});
...
$routes->post('become-author-post', 'AccountController::becomeAuthorPost');
$routes->post('report-bank-transfer-payment-post', 'PaymentController::reportBankTransferPayment');
...
$routes->group($customRoutes->admin, ['filter' => 'auth'], function ($routes) {
    ...
    $routes->match(['GET', 'POST'], 'author-applications', 'UserController::authorApplications');
...
if (!empty($languages)) {
    foreach ($languages as $language) {
        ...
        $routes->get($key . $customRoutes->checkout . '/' . $customRoutes->success, 'CheckoutController::paymentSuccess');
        $routes->get($key . $customRoutes->checkout . '/' . $customRoutes->pending, 'CheckoutController::paymentPending');
