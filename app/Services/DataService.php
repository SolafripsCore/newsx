...
class DataService
{
    ...
    public function getDashboardStats(): array
    {
        ...
        $cacheKey = $isSuperAdmin ? 'dashboard_heavy_stats_superadmin' : 'dashboard_heavy_stats_user_' . $userId;

        $cachedStats = $this->cacheService->get(
            $cacheKey,
            function () use ($userId, $isSuperAdmin) {

                $pageViewModel = model('PageViewsModel');

                $stats = [
                    'monthlyStatsForChart' => $pageViewModel->getMonthlyStatsForChart($userId),
                ];
        ...
        $liveStats = [
            'dashboardPostStats' => $postModel->getDashboardStats($isSuperAdmin ? null : $userId),
        ];
     ...
     */
    public function getWidgets(int $langId, ?bool $status = null): array
    {
        $statusKey = $status === true ? 'active' : 'all';
        $cacheKey  = 'widgets_lang_' . $langId . '_' . $statusKey;
        return $this->cacheService->get(
            $cacheKey,
            fn() => model('WidgetModel')->findAllByLang($langId, $status),
            'app_long'
        );
    }
