...
class PaymentService
{
    ...
    protected function processSubscription(object $order, array $gatewayData): void
    {
        ...
        $existingSubscriptions = $this->userSubscriptionModel
            ->where('user_id', $order->user_id)
            ->where('status', 'active')
            ->findAll();

        if (!empty($existingSubscriptions)) {
            foreach ($existingSubscriptions as $oldSub) {
                $this->cancelSubscriptionRecord($oldSub, 'Auto-cancelled due to plan change/upgrade.');
            }
        }
    ...
    /**
     * Creates a pending transaction record for asynchronous payment methods (e.g., NOWPayments, Bank Transfer)
     *
     * @param object $order
     ...
     * @return void
     */
    public function createPendingTransaction(object $order, string $gateway, ?string $transactionId = null): void
    {
        // Check if a transaction already exists
        $existing = $this->transactionModel->where('order_id', $order->id)->first();
        if (empty($existing)) {
            $transactionData = [
                'order_id'               => $order->id,
                'user_id'                => $order->user_id,
        ...
        } elseif (!empty($transactionId) && empty($existing->gateway_transaction_id)) {
            // If it exists but was missing the transaction ID (e.g., updated later), update it
            $this->transactionModel->update($existing->id, ['gateway_transaction_id' => $transactionId]);
        }
    }

    /**
     * Completes an asynchronous payment that was previously marked as pending
     *
     * @param object $order
     * @param array $gatewayData
     * @return bool
     */
    public function completeAsyncPayment(object $order, array $gatewayData): bool
    {
        ...
        try {
            ...
            $existingTx = $this->transactionModel
            ...
            if (!empty($existingTx)) {
                $this->transactionModel->update($existingTx->id, [
                    'gateway_transaction_id' => $gatewayData['transaction_id'] ?? $existingTx->gateway_transaction_id,
                    'status'                 => 'succeeded'
                ]);
            } else {
                // If no pending transaction exists, create one
                $transactionData = [
                    'order_id'               => $order->id,
                    'user_id'                => $order->user_id,
                    'gateway_transaction_id' => $gatewayData['transaction_id'] ?? null,
            ...
            log_message('error', '[PaymentService] Failed to complete async payment for Order ID: ' . $order->id . '. Error: ' . $e->getMessage());
            return false;
        }
    }

    /**
     * Cancels a user subscription both locally and at the payment gateway
     *
     * @param object $subscription The subscription database record
     * @param string $cancelReason The reason for cancellation (sent to gateway if applicable)
     * @return bool True if successfully cancelled, false otherwise
     */
    public function cancelSubscriptionRecord(object $subscription, string $cancelReason = 'Cancelled via system'): bool
    {
        $gatewayNameKey = strtolower($subscription->gateway_name_key);
        ...
        if (in_array($gatewayNameKey, ['stripe', 'paypal'])) {
            ...
            if (!empty($gatewayConfig) && !empty($subscriptionId)) {
                if ($gatewayNameKey === 'paypal') {
                    ...
                    $gatewayCancelSuccess = $paypalLib->cancelSubscription($subscriptionId, $cancelReason);
                } elseif ($gatewayNameKey === 'stripe') {
                    // Fetch application name from settings context
                    $appName = getContextValue('settings')->application_name ?? 'Our Platform';
                    $stripeLib = new \App\Libraries\Stripe($gatewayConfig, $appName);
        ...
        if (!$gatewayCancelSuccess) {
            log_message('error', "[PaymentService] Subscription cancellation failed at Gateway. Gateway: {$gatewayNameKey}, SubID: {$subscriptionId}");
            return false;
        }

        // Update local database status to cancelled
        $updateSuccess = $this->userSubscriptionModel->cancelSubscription($subscription->id);

        if ($updateSuccess) {
            log_message('info', "[PaymentService] Subscription successfully cancelled. SubID: {$subscriptionId}");
