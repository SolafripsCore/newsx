...
class AppContextService
{
    ...
    protected int $cacheTTL = 86400; // 1 day
    protected int $cacheTTLLong = 2592000; // 30 days
    ...
    // Feed Fallback Images
    protected array $feedFallbackImages = [];
    ...
    protected function setPremiumMembershipConfig(): void
    {
        ...
        $paywall = $settings->paywall_type ?? '';
        $paywallType = in_array($paywall, ['hard', 'preview']) ? $paywall : 'preview';
        ...
        $this->premiumMembership = (object)[
            ...
            'premiumBadgeVisibility'       => (int)($settings->premium_badge_visibility ?? 0),
        ];
    }
    ...
    /**
     * Set feed fallback image array
     */
    protected function setFeedFallbackImages(): void
    {
        $cacheKey = 'rss_feed_fallback_images';
        $fallbackImages = cache()->get($cacheKey);

        if ($fallbackImages !== null) {
            $this->feedFallbackImages = (array)$fallbackImages;
            return;
        }

        $feeds = $this->db->table('rss_feeds')
            ->select('rss_feeds.id AS feed_id, images.*')
            ->join('images', 'images.id = rss_feeds.image_id', 'inner')
            ->where('rss_feeds.image_id IS NOT NULL')
            ->get()
            ->getResult();

        $fallbackImages = [];
        foreach ($feeds as $feed) {
            $feedId = (int)$feed->feed_id;

            unset($feed->feed_id);

            $fallbackImages[$feedId] = $feed;
        }

        cache()->save($cacheKey, $fallbackImages, $this->cacheTTLLong);

        $this->feedFallbackImages = $fallbackImages;
    }
