...
class PaymentProcessorService
{
    ...
    /**
     * Processes and validates a Paystack transaction from the frontend callback
     *
     * @param object $order The order object from the database
     * @param string $reference The Paystack transaction reference (matches order_token)
     * @return array ['success' => bool, 'message' => string]
     */
    public function processPaystackPayment(object $order, string $reference): array
    {
        try {
            $config = $this->paymentGatewayModel->getGatewayConfig('paystack');
            if (empty($config) || (int)$config->is_active !== 1) {
                return ['success' => false, 'message' => 'Paystack gateway is currently unavailable.'];
            }

            $paystackLibrary = new \App\Libraries\Paystack($config);

            // Fetch and verify the payment directly from Paystack API
            $payment = $paystackLibrary->verifyTransaction($reference);

            // Verify payment status
            if ($payment && $payment->status === 'success') {

                // Validate amount and currency (Paystack amounts are in subunits / kobo)
                $paidAmountInSubunit = (int)$payment->amount;
                $paidCurrency = strtoupper($payment->currency);
                ...
                if ($paidAmountInSubunit !== $expectedAmountInSubunit || $paidCurrency !== $expectedCurrency) {
                    log_message('critical', "[Paystack Service] Amount/currency mismatch for Token: {$order->order_token}. Expected: {$expectedAmountInSubunit} {$expectedCurrency}, Paid: {$paidAmountInSubunit} {$paidCurrency}");
                    return ['success' => false, 'message' => 'Payment amount mismatch detected.'];
                }
                ...
                $gatewayData = [
                    'gateway'         => 'paystack',
                    'transaction_id'  => (string)$payment->id, // Paystack's internal transaction ID
                    'subscription_id' => null
                ];
            ...
            log_message('error', '[Paystack Service] Exception: ' . $e->getMessage());
            return ['success' => false, 'message' => trans("msg_error") ?? 'An error occurred during payment verification.'];
        }
    }
    ...
    /**
     * Processes and validates a NOWPayments crypto payment
     *
     * @param object $order The order object from the database
     * @param string $paymentId The NOWPayments payment ID
     * @param string $requestBody Required for IPN signature verification
     * @param string $signature Required for IPN signature verification
     * @param bool $isWebhook Flags if the request is coming from IPN
     * @return array ['status' => string, 'message' => string]
     */
    public function processNowPaymentsPayment(object $order, string $paymentId, string $requestBody = '', string $signature = '', bool $isWebhook = false): array
    {
        try {
            $config = $this->paymentGatewayModel->getGatewayConfig('nowpayments');

            if (empty($config) || (int)$config->is_active !== 1) {
                return ['status' => 'failed', 'message' => 'Payment gateway is currently unavailable.'];
            }

            $nowPaymentsLibrary = new \App\Libraries\NowPayments($config);

            // Verify IPN Signature (if webhook)
            if ($isWebhook && !empty($signature)) {
                if (!$nowPaymentsLibrary->verifyIpnSignature($requestBody, $signature)) {
                    log_message('warning', '[NOWPayments Service] IPN Signature verification failed. Falling back to strict API verification for ID: ' . $paymentId);
                }
            }

            // Ask NOWPayments directly using the payment_id
            $payment = $nowPaymentsLibrary->getPaymentStatus($paymentId);
            if (empty($payment) || !isset($payment->payment_status)) {
                return ['status' => 'failed', 'message' => 'Payment could not be fetched.'];
            }

            $status = strtolower($payment->payment_status);

            // Crypto payments pass through: waiting -> confirming -> sending -> finished
            if ($status !== 'finished') {

                // Prevent duplicate transaction records if the user refreshes the page
                $existingTx = $this->transactionModel->where('order_id', $order->id)->first();

                if (empty($existingTx)) {
                    // Create a pending transaction so the user sees a record while waiting for confirmation
                    $this->paymentService->createPendingTransaction($order, 'nowpayments', $paymentId);
                }

                return ['status' => 'pending', 'message' => "Payment is currently: {$status}. Waiting for blockchain confirmations."];
            }

            // Verify the order reference
            $externalReference = $payment->order_id ?? '';
            if ($externalReference !== $order->order_token) {
                log_message('critical', "[NOWPayments Service] Token mismatch. Expected: {$order->order_token}, Received: {$externalReference}");
                return ['status' => 'failed', 'message' => 'Order reference mismatch.'];
            }

            // Verify amount (Strict verification against price_amount)
            $paidAmount = number_format((float)($payment->price_amount ?? 0), 2, '.', '');
            $expectedAmount = number_format((float)$order->total_amount, 2, '.', '');
            $expectedCurrency = strtoupper($order->currency ?? 'USD');
            $paidCurrency = strtoupper($payment->price_currency ?? '');

            if (bccomp((string)$paidAmount, (string)$expectedAmount, 2) !== 0 || $paidCurrency !== $expectedCurrency) {
                log_message('critical', "[NOWPayments Service] Amount mismatch. Expected: {$expectedAmount} {$expectedCurrency}, Paid: {$paidAmount} {$paidCurrency}");
            ...
            // Use the asynchronous completion method to update the pending transaction and order
            $isProcessed = $this->paymentService->completeAsyncPayment($order, $gatewayData);

            if ($isProcessed) {
                return ['status' => 'completed', 'message' => 'Payment successful.'];
            }

            return ['status' => 'failed', 'message' => 'Database error occurred while processing order.'];

        } catch (\Exception $e) {
            log_message('error', '[NOWPayments Service] Exception: ' . $e->getMessage());
            return ['status' => 'failed', 'message' => trans("msg_error") ?? 'An error occurred during payment verification.'];
        }
    }
}