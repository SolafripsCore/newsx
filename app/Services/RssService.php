...
class RssService
{
    ...
    public function importFeed(object $feed): array
    {
        ...
        foreach ($items as $item) {
            ...
            try {
                ...
                // Validate the image URL; clear it if the format is invalid to trigger the fallback
                ...
                if (isset($feed->image_saving_method) && $feed->image_saving_method === 'download' && !empty($imageUrl)) {
                    $downloadedImageId = $this->processImages($imageUrl, (int)$feed->user_id);

                    // If the download is successful, overwrite the fallback image ID with the new one
                    if (!empty($downloadedImageId)) {
                        $post->image_id = $downloadedImageId;
                        $post->image_url = '';
                    }
                }
    ...
    /**
     * Validates if a URL is structurally a valid image link without making HTTP requests.
     * Checks for valid URL format, allowed schemes, and supported image extensions.
     *
     * @param string $url
     * @return bool
     */
    private function isValidImageUrl(string $url): bool
    {
        ...
        // Safe casting to string to prevent PHP 8 deprecation warnings if scheme is null
        if (empty($scheme) || !in_array(strtolower((string)$scheme), ['http', 'https'])) {
            return false;
        }
