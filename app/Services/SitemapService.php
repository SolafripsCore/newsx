...
class SitemapService
{
    ...
    private function getCategoriesSource(): Generator
    {
        ...
        do {
            $cats = $categoryModel->select('id, slug, parent_slug, lang_id, updated_at, created_at')
                ->where('id >', $lastId)
                ->orderBy('id', 'ASC')
                ->findAll(self::DB_CHUNK_SIZE);
            ...
            foreach ($cats as $cat) {
                $lastId = (int)$cat->id;
                $language = $this->languages[$cat->lang_id] ?? null;

                // Ensure the base URL always ends with a single slash
                $rawBaseUrl = $language['base_url'] ?? base_url();
                $baseUrl = rtrim((string)$rawBaseUrl, '/') . '/';

                if (!empty($cat->parent_slug)) {
                    $catUrl = $baseUrl . $cat->parent_slug . '/' . $cat->slug;
                } else {
