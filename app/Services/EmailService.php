...
class EmailService
{
    ...
    /**
     * Sends an email regarding author role requests based on the action type.
     *
     * @param string $actionType The type of action ('new_request', 'approved', 'soft_rejected', 'hard_rejected').
     * @param string $recipientEmail The email address of the recipient (Admin or User).
     * @param array $params Additional data for the email (e.g., 'name', 'user_email', 'reason').
     * @return bool True if successful, false otherwise.
     */
    public function sendAuthorRequestEmail(string $actionType, string $recipientEmail, ?array $params = []): bool
    {
        if (empty($recipientEmail) || !filter_var($recipientEmail, FILTER_VALIDATE_EMAIL)) {
            return false;
        }

        // Sanitize parameters to prevent XSS in email body
        $userName = htmlspecialchars($params['name'] ?? 'User');
        $userEmail = htmlspecialchars($params['user_email'] ?? 'N/A');
        $becomeAuthorPageUrl = htmlspecialchars($params['become_author_page_url'] ?? base_url());

        // Optional rejection/revision reason provided by the admin
        $reasonRaw = trim($params['reason'] ?? '');
        $reasonHtml = !empty($reasonRaw) ? nl2br(htmlspecialchars($reasonRaw)) : '';

        $reasonBlock = '';
        if ($reasonHtml !== '') {
            $reasonColor = ($actionType === 'hard_rejected') ? '#ef4444' : '#f59e0b';
            $reasonBlock = "<div style='background-color: #f9fafb; padding: 15px; border-left: 4px solid " . $reasonColor . "; margin: 20px 0;'>
                                <p style='margin:0;'><strong>Reviewer Feedback:</strong><br/>" . $reasonHtml . "</p>
                            </div>";
        }

        // Determine email content
        $emailContent = match ($actionType) {
            'new_request' => [
                'subject'    => trans("author_app_email_new_author_request"),
                'title'      => trans("author_app_email_new_author_request"),
                'body'       => "<p>" . trans("author_app_email_new_author_request_exp") . "</p>
                           <p><strong>" . trans("name") . ":</strong> " . $userName . "</p>
                           <p><strong>" . trans("email") . ":</strong> " . $userEmail . "</p>",
                'buttonText' => trans("author_applications"),
                'buttonUrl'  => adminUrl("author-applications")
            ],
            'approved' => [
                'subject'    => trans("author_app_email_your_request_approved"),
                'title'      => trans("author_app_email_your_request_approved"),
                'body'       => "<p>" . trans("author_app_email_your_request_approved_exp") . "</p>",
                'buttonText' => trans("go_to_dashboard"),
                'buttonUrl'  => adminUrl()
            ],
            'soft_reject' => [
                'subject'    => trans("author_app_email_update_need_request"),
                'title'      => trans("author_app_email_update_need_request"),
                'body'       => "<p>" . trans("author_app_email_revision_need_request_exp") . "</p>" . $reasonBlock,
                'buttonText' => trans("author_app_email_update_your_request"),
                'buttonUrl'  => $becomeAuthorPageUrl
            ],
            'hard_reject' => [
                'subject'    => trans("author_app_email_your_request_rejected"),
                'title'      => trans("author_app_email_your_request_rejected"),
                'body'       => "<p>" . trans("author_app_email_your_request_rejected_exp") . "</p>" . $reasonBlock,
                'buttonText' => '',
                'buttonUrl'  => ''
            ],
            default => null,
