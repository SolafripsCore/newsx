...
class CheckoutController extends BaseController
{
    ...
    private function initiatePayment(object $paymentGateway, object $order, array $data): array
    {
        try {
            switch ($paymentGateway->name_key) {
                ...
                case 'paystack':
                    $data = $this->initiatePaystackPayment($paymentGateway, $order, $data);
                    break;
                ...
                case 'nowpayments':
                    $data = $this->initiateNowPaymentsPayment($paymentGateway, $order, $data);
                    break;
     ...
     */
    private function initiatePaystackPayment(object $paymentGateway, object $order, array $data): array
    {
        $paystackLibrary = new \App\Libraries\Paystack($paymentGateway);

        $customerEmail = !empty($order->billing_email) ? $order->billing_email : user()->email;
        $callbackUrl = base_url('checkout/payment/paystack') . '?reference=' . $order->order_token;

        $paystackData = $paystackLibrary->initializeTransaction($order, $customerEmail, $callbackUrl);
        if (empty($paystackData) || empty($paystackData->authorization_url)) {
            throw new \Exception(trans("payment_option_load_error") ?? "Paystack could not generate an authorization URL.");
        }

        $data['paystackAuthorizationUrl'] = $paystackData->authorization_url;
    ...
    /**
     * Initializes the NOWPayments crypto checkout flow
     */
    private function initiateNowPaymentsPayment(object $paymentGateway, object $order, array $data): array
    {
        $nowPaymentsLibrary = new \App\Libraries\NowPayments($paymentGateway);

        $successUrl = base_url('checkout/payment/nowpayments') . '?order_token=' . $order->order_token;
        $cancelUrl = $data['backUrl'];
        $ipnUrl = base_url('webhook/nowpayments');

        $nowPaymentsCheckoutUrl = $order->payment_url ?? null;

        if (empty($nowPaymentsCheckoutUrl)) {
            $invoice = $nowPaymentsLibrary->createInvoice($order, $successUrl, $cancelUrl, $ipnUrl);

            if (empty($invoice) || empty($invoice->invoice_url)) {
                throw new \Exception(trans("payment_option_load_error"));
            }

            $nowPaymentsCheckoutUrl = $invoice->invoice_url;

            // Persist the generated URL to prevent duplicate generation attempts
            $this->orderModel->update($order->id, [
                'payment_url' => $nowPaymentsCheckoutUrl
            ]);
        }

        $data['paymentUrl'] = $nowPaymentsCheckoutUrl;
    ...
    public function paymentSuccess()
    {
        ...
        return loadCommonView('checkout/payment_success', $data);
    }
    ...
    public function paymentPending()
    {
        ...
        // If it's cancelled or any other status except pending, redirect to home
        if ($order->status !== 'pending') {
            return redirect()->to(langBaseUrl());
        }

        // Fetch the associated pending transaction to identify the gateway
        $transaction = model('TransactionModel')->where('order_id', $order->id)->first();
        $gateway = $transaction->payment_method ?? '';

        // Fetch bank accounts data dynamically if it's a bank transfer
        $gatewayData = '';
        if ($gateway === 'bank_transfer') {
            $bankConfig = model('PaymentGatewayModel')->getGatewayConfig('bank_transfer');
            $gatewayData = $bankConfig->gateway_data ?? '';
        }

        $data = [
            'title'       => trans("order_received"),
            'description' => trans("payment_is_pending_exp"),
            'itemName'    => $order->item_title ?? ($order->item_type === 'subscription' ? 'Premium Subscription' : 'Premium Content'),
            'amount'      => numToDecimal($order->total_amount),
            'gateway'     => $gateway,
            'referenceId' => strtoupper(substr($order->order_token, 0, 8)),
            'gatewayData' => $gatewayData
        ];

        $data = setPageMeta(trans("order_received"), $data);

        return loadCommonView('checkout/payment_pending', $data);
    }
}