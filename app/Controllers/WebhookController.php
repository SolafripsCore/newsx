...
class WebhookController extends BaseController
{
    ...
    public function paystack()
    {
        try {
            ...
            if (empty($config) || empty($config->secret_key)) {
                log_message('critical', '[Paystack Webhook] Secret key is not configured.');
                return $this->response->setStatusCode(500)->setBody('Server Error');
            }

            $paystackLibrary = new \App\Libraries\Paystack($config);
            $parsedData = $paystackLibrary->parseWebhookPayload($requestBody, $signature);

            // Invalid Signature or Missing Data
            if (!$parsedData['isValid']) {
                log_message('error', '[Paystack Webhook] ' . $parsedData['error']);
                return $this->response->setStatusCode(400)->setBody('Bad Request');
            }

            // Valid Signature but irrelevant event (e.g., subscription update)
            if ($parsedData['isIgnored']) {
                return $this->response->setStatusCode(200)->setBody('Event Ignored');
            }

            // Valid 'charge.success' event
            $reference = $parsedData['reference'];
            $order = $this->orderModel->where('order_token', $reference)->first();

            if (empty($order)) {
                log_message('error', "[Paystack Webhook] Order not found for reference: {$reference}");
                return $this->response->setStatusCode(200)->setBody('Order Not Found');
            }
            ...
            if (!$result['success']) {
                log_message('error', "[Paystack Webhook] Failed to process payment for reference {$reference}: " . $result['message']);
            }

            return $this->response->setStatusCode(200)->setBody('OK');

        } catch (\Exception $e) {
            log_message('critical', '[Paystack Webhook] Fatal Error: ' . $e->getMessage());
            return $this->response->setStatusCode(500)->setBody('Internal Server Error');
        }
    }
    ...
    /**
     * Handles the Instant Payment Notification (IPN) from NOWPayments
     *
     * @return \CodeIgniter\HTTP\ResponseInterface
     */
    ...
    {
        try {
            // Capture the raw request body and signature
            $requestBody = $this->request->getBody();
            $signature = $this->request->getHeaderLine('x-nowpayments-sig');

            // Decode the payload
            $postData = json_decode($requestBody, true);

            // Validate the basic payload structure
            if (empty($postData) || empty($postData['payment_id'])) {
                log_message('error', '[NOWPayments Webhook] Invalid payload received.');
                return $this->response->setStatusCode(400)->setBody('Invalid Payload');
            }

            $orderToken = $postData['order_id'] ?? '';
            $paymentId = (string)$postData['payment_id'];

            // Fetch the order from the database
            $order = $this->orderModel->where('order_token', $orderToken)->first();

            if (empty($order)) {
                log_message('error', "[NOWPayments Webhook] Order not found for token: {$orderToken}");
                return $this->response->setStatusCode(200)->setBody('Order Not Found - Ignored');
            }
            ...
            // Delegate to the processor (Double-Check Architecture)
            $result = $this->paymentProcessorService->processNowPaymentsPayment($order, $paymentId, $requestBody, $signature, true);

            if ($result['status'] === 'failed') {
                log_message('error', "[NOWPayments Webhook] Failed to process payment {$paymentId}: " . $result['message']);
            }

            // Always return 200 OK to acknowledge receipt, unless it's a critical server failure
            return $this->response->setStatusCode(200)->setBody($result['message'] ?? 'OK');

        } catch (\Exception $e) {
            log_message('critical', '[NOWPayments Webhook] Fatal Error: ' . $e->getMessage());
            return $this->response->setStatusCode(500)->setBody('Internal Server Error');
        }
    }
}