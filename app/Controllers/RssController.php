...
{
    protected $rssModel;
    protected $fallbackImagesCacheKey = 'rss_feed_fallback_images';
    ...
    public function addFeed()
    {
        if (isPostMethod()) {
            ...
            if ($this->rssModel->trySave($feed)) {
                ...
                // Clear fallback image cache
                cache()->delete($this->fallbackImagesCacheKey);
    ...
    public function editFeed($id)
    {
        ...
        // Default Image
        $defaultImage = null;
        if (!empty($feed->image_id)) {
            $defaultImage = model('ImageModel')->find($feed->image_id);
        }
