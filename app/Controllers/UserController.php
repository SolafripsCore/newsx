...
class UserController extends BaseAdminController
{
    ...
    public function authorApplications()
    {
        ...
        $model = model('AuthorApplicationModel');

        if (isPostMethod()) {
            $postData = $this->request->getPost();
            $action = $postData['action'] ?? '';
            $applicationId = (int)($postData['id'] ?? 0);

            if ($action === 'update_request') {
                $userId = (int)($postData['user_id'] ?? 0);
                $rewardSystem = (int)($postData['reward_system'] ?? 0);
                $status = (string)($postData['status'] ?? '');
                $reviewerFeedback = (string)($postData['reviewer_feedback'] ?? '');

                // Update the application status
                if ($model->updateApplicationStatus($applicationId, $status, $reviewerFeedback)) {
                    $userModel = model('UserModel');

                    // If approved, set user role
                    if ($status === 'approved' && $userId > 0) {
                        $userModel->changeUserRole([
                            'id'      => $userId,
                            'role_id' => 2
                        ]);
                        ...
                        if ($rewardSystem === 1) {
                            $userModel->update($userId, ['reward_system' => 1]);
                        }
                    }
                    ...
                    if ($userId > 0) {
                        ...
                        if (!empty($user)) {
                            ...
                            $becomeAuthorPageUrl = base_url();
                            $becomeAuthorPage = model('PageModel')->findByDefaultName('become_an_author', (int)$this->config->site_lang);
                            if (!empty($becomeAuthorPage)) {
                                $becomeAuthorPageUrl = generateURL($becomeAuthorPage->slug);
                            }

                            $params = [
                                'name'                   => $user->username ?? 'User',
                                'user_email'             => $user->email,
                                'reason'                 => $reviewerFeedback,
                                'become_author_page_url' => $becomeAuthorPageUrl
                            ];

                            if (!empty($this->config->mail_contact)) {
                                $emailService = new \App\Services\EmailService();
                                $emailService->sendAuthorRequestEmail($status, $user->email, $params);
                            }
                        }
                    }
                ...
                $model->delete($applicationId) ? setSuccessMessage("msg_deleted") : setErrorMessage("msg_error");
                return jsonResponse(true);
            }
        ...
        $data = [
            'title'         => trans("author_applications"),
            'action'        => adminUrl("author-applications"),
            'panelSettings' => panelSettings(),
            'applications'  => $model->findAllPaginated($this->perPage),
            'pager'         => $model->pager
        ];

        return view('admin/user/author_applications', $data);
    }
