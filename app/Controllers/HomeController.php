...
class HomeController extends BaseController
{
    ...
    private function post($post)
    {
        ...
        $paywallType = $this->premiumMembership->paywallType ?? '';
        ...
        $view = !$premiumAccessStatus->hasAccess && $paywallType === 'hard' ? 'post_hard_paywall' : 'post';
    ...
    private function page($page)
    {
        ...
        elseif ($page->page_default_name === 'become_an_author') {

            $data['authorApplication'] = authCheck() ? model('AuthorApplicationModel')->getApplicationByUserId((int)user()->id) : null;
            $data['user'] = authCheck() ? user() : null;

            return loadCommonView('account/become_author', $data);
        }
    ...
    {
        $contactPage = model('PageModel')->findByDefaultName('contact', (int)$this->activeLang->id);

        $redirectUrl = generateURL('contact');
        if (!empty($contactPage) && $contactPage->slug) {
            $redirectUrl = generateURL($contactPage->slug);
        }
        ...
        if (model('ContactModel')->create($postData)) {
