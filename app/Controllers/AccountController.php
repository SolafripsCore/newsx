...
class AccountController extends BaseController
{
     ...
     * Become Author Post
     */
    public function becomeAuthorPost()
    {
        if (!authCheck()) {
        ...
        $user->first_name = !empty($postData['first_name']) ? trim($postData['first_name']) : '';
        $user->last_name = !empty($postData['last_name']) ? trim($postData['last_name']) : '';

        if ($this->userModel->trySave($user)) {
            // Prepare form data
            $portfolioUrl = !empty($postData['portfolio_url']) ? trim($postData['portfolio_url']) : '';
            $aboutExpertise = !empty($postData['about_expertise']) ? trim($postData['about_expertise']) : '';

            if (model('AuthorApplicationModel')->submitApplication($userId, $portfolioUrl, $aboutExpertise)) {

                // Send email
                if (!empty($this->config->mail_contact)) {
                    $emailService = new \App\Services\EmailService();
                    $emailService->sendAuthorRequestEmail('new_request', $this->config->mail_contact, [
