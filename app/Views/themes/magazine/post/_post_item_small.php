...
$hasImage = postHasImage($postItem);
...
<div class="d-flex align-items-start post-item-small mb-4">
    <div class="image">
        <a href="<?= $postUrl; ?>" class="flex-shrink-0" aria-label="<?= esc($postItem->title); ?>"<?= postUrlNewTab($postItem); ?>>
            <img src="<?= $imgUrl; ?>" alt="<?= esc($imgAltText); ?>" class="img-fluid" width="128" height="90" loading="lazy"
                    <?= getFeedFallbackErrorAttr($postItem, 'small'); ?>/>
            <?= getPostBadges($postItem, 'media-sm'); ?>
        </a>
    </div>

    <div class="flex-grow-1 ms-3">
        <h3 class="title">
            <a href="<?= $postUrl; ?>"<?= postUrlNewTab($postItem); ?> class="d-block"><?= getDisplayTitle($postItem->title); ?></a>
        </h3>
