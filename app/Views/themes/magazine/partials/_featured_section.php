<section class="section featured-section">
    <div class="container-xl">
        <div class="fs-grid-wrapper">
                                    ...
                                    <div class="swiper-slide">
                                        <div class="fs-card">
                                            ...
                                            <img src="<?= getPostImageUrl($item, 'slider'); ?>" alt="<?= esc(!empty($item->alt_text) ? $item->alt_text : $item->title); ?>" class="fs-img" <?= $count === 0 ? 'fetchpriority="high"' : 'loading="lazy"'; ?>
                                                    <?= getFeedFallbackErrorAttr($item, 'slider'); ?>/>
                                            <?= getPostBadges($item, 'media-lg'); ?>
            ...
            <?php
            $featTop = $featuredPosts[0] ?? null;
            $featBottom = !empty($featuredPosts) ? array_slice($featuredPosts, 1, 2) : [];
            ?>
            <div class="fs-grid-item fs-grid-boxes area-sub-top">
                <?php if ($featTop): ?>
                    <div class="fs-card">
                        <a href="<?= generatePostUrl($featTop); ?>" class="fs-main-link" <?= postUrlNewTab($featTop); ?> aria-label="<?= esc($featTop->title); ?>"></a>
                        <img src="<?= getPostImageUrl($featTop, 'big'); ?>" alt="<?= esc(!empty($featTop->alt_text) ? $featTop->alt_text : $featTop->title); ?>" class="fs-img" loading="lazy"
                                <?= getFeedFallbackErrorAttr($featTop, 'big'); ?>/>
                        <?= getPostBadges($featTop); ?>
                        <div class="fs-content">
                            <a href="<?= generateCategoryUrl($featTop); ?>" class="fs-badge" title="<?= esc($featTop->cat_name); ?>">
                                <span class="badge badge-category" style="background-color: <?= esc($featTop->cat_color); ?>">
                                    <?= esc($featTop->cat_name); ?>
                                </span>
                            </a>
                            <h3 class="fs-title">
                                <a href="<?= generatePostUrl($featTop); ?>"><?= esc(getDisplayTitle($featTop->title, 90)); ?></a>
                            </h3>
                            <div class="fs-meta">
                                <?= loadView("post/_post_meta", ['postItem' => $featTop]); ?>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
            ...
            <div class="fs-grid-item fs-grid-boxes area-sub-bottom">
                <div class="sub-grid">
                    <?php foreach ($featBottom as $item): ?>
                        <div class="fs-card fs-card-small">
                            <a href="<?= generatePostUrl($item); ?>" class="fs-main-link" <?= postUrlNewTab($item); ?> aria-label="<?= esc($item->title); ?>"></a>
