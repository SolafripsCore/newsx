...
foreach (['primary', 'secondary'] as $target) {
    ...
    $font = $activeFonts[$target];

    // Define the baseline fallback
    $path400 = isFontValid($font->path_400 ?? null) ? $font->path_400 : null;

    // Attempt specific weight, fallback to 400 if invalid
    $path600 = isFontValid($font->path_600 ?? null) ? $font->path_600 : $path400;
    $path700 = isFontValid($font->path_700 ?? null) ? $font->path_700 : $path400;
    ...
    foreach ($weightsToPreload as $weight => $path) {
        if (empty($path)) {
            continue;
        }
...
foreach ($activeFonts as $font) {
    ...
    foreach ($weightsToCss as $weight => $path) {
        if (empty($path)) {
            continue;
        }
