        ...
        <?php if (!empty($largePost)): ?>
            <div class="post-featured ratio ratio-16x9">
                <a href="<?= generatePostUrl($largePost); ?>" aria-label="<?= esc($largePost->title); ?>"<?= postUrlNewTab($largePost); ?>>
                    <?php if (postHasImage($largePost)):
                        $imgAltText = !empty($largePost->alt_text) ? $largePost->alt_text : $largePost->title; ?>
                        <img src="<?= getPostImageUrl($largePost, 'big'); ?>" alt="<?= esc($imgAltText); ?>" class="img-fluid img-post w-100 h-100 object-fit-cover" width="756" height="425" loading="lazy" <?= getFeedFallbackErrorAttr($largePost, 'big'); ?>/>
                        <?= getPostBadges($largePost, 'media-lg'); ?>
                    <?php endif; ?>
                </a>
                <div class="caption">
