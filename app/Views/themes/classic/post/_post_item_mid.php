    ...
    if (checkPostImg($postItem)): ?>
        <div class="image ratio">
            <a href="<?= generatePostUrl($postItem); ?>"<?= postUrlNewTab($postItem); ?>>
                <img src="<?= getPostImageUrl($postItem, 'mid'); ?>" alt="<?= esc($imgAltText); ?>" class="img-fluid" width="306" height="182" loading="lazy" <?= getFeedFallbackErrorAttr($postItem, 'mid'); ?>/>
                <?= getPostBadges($postItem, 'media-md'); ?>
            </a>
        </div>
    <?php endif; ?>
