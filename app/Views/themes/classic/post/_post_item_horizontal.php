...
<div class="post-item-horizontal<?= esc($extraClass); ?>">
    <div class="row">
            ...
            <div class="col-12 col-sm-5 position-relative">
                ...
                <div class="image ratio">
                    <a href="<?= $postUrl; ?>" aria-label="<?= esc($postItem->title); ?>"<?= postUrlNewTab($postItem); ?>>
                        <img src="<?= $imgUrl; ?>" alt="<?= esc($imgAltText); ?>" class="img-fluid h-100 object-fit-cover" width="350" height="225" loading="lazy" <?= getFeedFallbackErrorAttr($postItem, 'mid'); ?>/>
                        <?= getPostBadges($postItem); ?>
                    </a>
                </div>
            </div>
        <?php else: ?>
        ...
        <div class="col-12 <?= $hasImage ? 'col-sm-7' : ''; ?>">
            <h3 class="title">
