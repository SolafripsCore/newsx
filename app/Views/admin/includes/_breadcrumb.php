...
if (!empty($breadcrumbLink) && !empty($breadcrumbLink['label']) && !empty($breadcrumbLink['url'])) {
    array_unshift($links, ['label' => $breadcrumbLink['label'], 'url' => $breadcrumbLink['url']]);
}

array_unshift($links, ['label' => trans("home"), 'url' => adminUrl()]); ?>
...
<div class="d-flex justify-content-between align-items-center flex-wrap mb-6">
            ...
            <?php foreach ($links as $index => $link): ?>
                <?php if (!empty($link['url'])): ?>
                    <li class="breadcrumb-item text-muted">
                        <a href="<?= $link['url'] ?>" class="text-muted text-hover-primary">
                            <?= esc($link['label']) ?>
                        </a>
                    </li>
                <?php else: ?>
    ...
    <div class="d-flex gap-3">
        ...
        <?php if (!empty($breadcrumbInvoiceLink)): ?>
            <a href="<?= esc($breadcrumbInvoiceLink); ?>" class="btn btn-success" target="_blank">
                <i class="ki-duotone ki-document fs-2"><span class="path1"></span><span class="path2"></span></i>
                <?= trans("invoice"); ?>
            </a>
        <?php endif; ?>

        <?php if (!empty($aiWriter->status) && !empty($aiGenerateType) && $aiGenerateType === 'all'): ?>
            <button type="button" class="btn btn-success gap-2" data-bs-toggle="modal" data-bs-target="#modalAiWriter">
                ...
                <?= trans("generate_with_ai"); ?>
            </button>
        <?php endif; ?>

        <?php if (!empty($breadcrumbLink) && !empty($breadcrumbLink['label']) && !empty($breadcrumbLink['url'])): ?>
            <a href="<?= $breadcrumbLink['url']; ?>" class="btn btn-primary">
                <i class="bi bi-list fs-2 me-1"></i><?= esc($breadcrumbLink['label']); ?>
            </a>
        <?php endif; ?>
    </div>
