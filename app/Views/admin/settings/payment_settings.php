...
$labelMap = [
        ...
        'paystack'     => ['public_key' => 'public_key', 'secret_key' => 'secret_key'],
        'mercado_pago' => ['public_key' => 'public_key', 'secret_key' => 'secret_key'],
        'nowpayments'  => ['public_key' => 'public_key', 'secret_key' => 'api_key'],
];
                        ...
                        <?php foreach ($gateways as $gateway) : ?>
                            <div class="tab-pane position-relative fade show <?= $activeTab == $gateway->name_key ? 'active' : ''; ?>" id="tab_<?= $gateway->id; ?>" role="tabpanel" data-tab="general">
                                ...
                                <form action="<?= $action; ?>" method="post" class="form">
                                    ...
                                    <?php if ($gateway->name_key != 'bank_transfer' && !in_array($gateway->name_key, ['paystack', 'nowpayments'])): ?>
                                        <div class="row mb-6">
                                    ...
                                    <?php if ($gateway->name_key != 'bank_transfer'): ?>
                                        <div class="row mb-6">
                                    ...
                                    <?php if ($gateway->name_key != 'bank_transfer' && in_array($gateway->name_key, ['paypal', 'stripe', 'razorpay', 'nowpayments'])):
                                        $webhookSecretLabel = $gateway->name_key === 'nowpayments' ? trans("secret_key") . " (IPN)" : trans("webhook_secret_id"); ?>
                                        <div class="row mb-6">
                                            <label class="col-lg-5 col-form-label fw-semibold fs-6"><?= esc($webhookSecretLabel); ?></label>
                                            <div class="col-lg-7 d-flex align-items-center">
                                                <input type="text" name="webhook_secret" value="<?= esc($gateway->webhook_secret); ?>" class="form-control" placeholder="<?= esc($webhookSecretLabel, "attr"); ?>" maxlength="500"/>
                                            </div>
                                        </div>
                                    <?php endif; ?>
                                    ...
                                    <?php if ($gateway->name_key == 'bank_transfer'): ?>
                                        <div class="row mb-6">
                                            <label class="form-label"><?= trans("bank_accounts"); ?></label>
                                            <div>
                                                <?= view('admin/includes/_text_editor', [
                                                        'edtName'     => 'gateway_data',
                                                        'edtValue'    => $gateway->gateway_data,
                                                        'edtImage'    => false,
                                                        'edtAIWriter' => false,
                                                        'edtClass'    => 'tinyMCEMid'
                                                ]); ?>
