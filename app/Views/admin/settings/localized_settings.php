                ...
                <div class="tab-content" id="tabContent">
                    <div class="tab-pane fade show <?= $activeTab == 'general' ? 'active' : ''; ?>" id="tab_general" role="tabpanel">
                        <form action="<?= adminUrl('localized-settings'); ?>" method="post" class="form" enctype="multipart/form-data">
                            ...
                            <div class="row mb-6">
                                <label class="col-lg-4 col-form-label required fw-semibold fs-6"><?= trans('date_format'); ?></label>
                                <div class="col-lg-8 fv-row">
                                    <?php
                                    $dateFormats = getAppDefault('dateFormats');
                                    $isCustomSelected = !array_key_exists($settings->date_format, $dateFormats);
                                    ?>

                                    <select name="date_format" id="dateFormatSelect" class="form-select" data-control="select2" data-hide-search="true" data-placeholder="<?= trans("select_an_option", "attr"); ?>">
                                        <?php foreach ($dateFormats as $key => $value): ?>
                                            <option value="<?= esc($key); ?>" <?= $settings->date_format === $key ? 'selected' : ''; ?>>
                                                <?= esc($key); ?> &nbsp;(<?= esc($value); ?>)
                                            </option>
                                        <?php endforeach; ?>
                                        <option value="custom" <?= $isCustomSelected ? 'selected' : ''; ?>>
                                            <?= trans('custom'); ?>...
                                        </option>
                                    </select>

                                    <div id="customFormatContainer" class="mt-3 <?= $isCustomSelected ? '' : 'd-none'; ?>">
                                        <input type="text" name="custom_date_format" id="customFormatInput" class="form-control mw-300px" value="<?= esc($settings->date_format); ?>" placeholder="e.g. j F Y" autocomplete="off">
                                    </div>
                                </div>
                            </div>
