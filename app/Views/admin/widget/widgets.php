...
<?= $this->section('content'); ?>
    ...
    <div class="card">
        <form action="<?= adminUrl('widgets'); ?>" method="get" class="form-filter">
                ...
                <div class="card-toolbar">
                    <div class="d-flex justify-content-end gap-3" data-kt-subscription-table-toolbar="base">
                        <?= view('admin/includes/_filters', [
                                'filterPageUrl' => adminUrl('widgets'),
                                'filters'       => [
                                        'language',
                                        'status',
                                        'search' => trans("title")
                        ...
                        <a href="<?= adminUrl('widgets/add'); ?>" class="btn btn-primary">
            ...
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-5">
                    ...
                    <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                        ...
                        <th class="min-w-125px"><?= trans('where_to_display'); ?></th>
                        <th class="min-w-125px"><?= trans('order_1'); ?></th>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="text-muted fs-7">
                                        <?= (int)$item->is_custom === 1 ? trans('custom') : trans("default"); ?>
                                ...
                                <td>
                                    <?php
                                    if (!empty($item->display_category_id)) {
                                        echo esc($item->category_name);
                                    } else {
                                        if (!empty($latestPostsTranslationArray[$item->lang_id])) {
                                            echo $latestPostsTranslationArray[$item->lang_id];
                                        } else {
                                ...
                                <td class="text-end">
                                    ...
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 menu-table-options py-4" data-kt-menu="true">
                                        <div class="menu-item px-3">
                                            <a href="<?= adminUrl('widgets/edit/' . esc($item->id)); ?>" class="menu-link px-3"><?= trans('edit'); ?></a>
                                        </div>
                                        <div class="menu-item px-3">
                                            <a href="javascript:void(0)" class="menu-link px-3 text-danger js-action-trigger"
                                               data-url="<?= base_url('Admin/deleteWidget'); ?>"
                                               data-action="delete"
                                               data-id="<?= $item->id; ?>"
