...
class WidgetModel extends BaseModel
{
    ...
    /**
     * Finds all widgets by language
     */
    public function findAllByLang(int $langId, ?bool $status = null): array
    {
        $this->where('lang_id', $langId);

        if ($status === true) {
            $this->where('status', 1);
        }

        return $this->orderBy('widget_order', 'ASC')->findAll();
    }
